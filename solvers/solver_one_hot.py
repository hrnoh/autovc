...
class Solver(object):
    ...
    def build_model(self):
        ...
        self.G = Generator(hparams.dim_neck, hparams.speaker_num, hparams.dim_pre, hparams.freq).to(self.device)
    ...
    def train(self):
        ...
        while self.global_epoch < hparams.nepochs:
            for mels, _, labels_onehot in train_loader:
                ...
                labels = labels_onehot.to(self.device)

                mel_outputs, mel_outputs_postnet, code = self.G(mels, labels, labels)
                ...
                recon_code = self.G(mel_outputs_postnet, labels, None)

                loss_content = torch.nn.functional.l1_loss(recon_code, code)

                loss_total = loss_recon + hparams.recon_mu * loss_init_recon + hparams.content_lambda * loss_content
